...
# Make sure ${OUTDIR} is created directory
if [ ! -d ${OUTDIR} ]
then
...
if [ ! -e ${OUTDIR}/linux-stable/arch/${ARCH}/boot/Image ]; then
    ...
    # Kernel build steps
    make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} mrproper    # Deep clean prev configs
    make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} defconfig   # Configure virt arm dev board
    make -j4 ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} all     # Build vmlinux target, kernel image
